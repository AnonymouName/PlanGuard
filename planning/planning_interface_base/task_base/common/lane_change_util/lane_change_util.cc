...
//wk
bool IsClearToChangeLane(ReferenceLineInfo* reference_line_info, const double& distance = 0.0, const double& if_check = 0.0) {
  ...
  for (const auto* obstacle :
    ...
    //wk
    kForwardSafeDistance = (distance > 0.0)? distance : kForwardSafeDistance;
    ...
    if (HysteresisFilter(ego_start_s - end_s, kBackwardSafeDistance,
        ...
        HysteresisFilter(start_s - ego_end_s, kForwardSafeDistance,
                         kDistanceBuffer, obstacle->IsLaneChangeBlocking()) && if_check != 2.0) {
