...
class OnLanePlanning : public PlanningBase {
 public:
  ...
  void RunOnce(const LocalView& local_view,
               ...
               UserIntercation* const user_intercation) override;
 ...
 private:
  ...
  bool ChangeLane();
  void UpdateFLAGS();
//   bool Update_trajectory(ADCTrajectory* const ptr_trajectory_pb);
  void envrionment_monitor();
  std::vector<apollo::common::PointENU> GetPolygon(apollo::common::PointENU original_point, double heading, 
  ...
  double CalculateDistanceFromObstacleToVehicle(std::vector<apollo::common::PointENU> ego, std::vector<apollo::common::PointENU> obj);
  double CalculationDistanceToALine(double x, double y, double start_x, double start_y, double end_x, double end_y);
  bool CheckWhetherJunctionisAhead(apollo::hdmap::Id the_id, double ego_heading, double x, double y);
  int doRectangleAndPolygonIntersect(double x, double y, double start_x, double start_y, double end_x, double end_y);
 private:
  ...
  //wk
  std::mutex user_intercation_mutex_;
};
