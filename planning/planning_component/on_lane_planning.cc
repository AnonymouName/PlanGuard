...
Status OnLanePlanning::InitFrame(const uint32_t sequence_num,
  ...
  frame_.reset(new Frame(sequence_num, local_view_, planning_start_point,
                         vehicle_state, reference_line_provider_.get(), user_intercation_));
...
void OnLanePlanning::RunOnce(const LocalView& local_view,
                             ADCTrajectory* const ptr_trajectory_pb,
                             UserIntercation* const user_intercation) {
  // when rerouting, reference line might not be updated. In this case, planning
  // module maintains not-ready until be restarted.
  local_view_ = local_view;
  //wk
  user_intercation_ = user_intercation;
  const double start_timestamp = Clock::NowInSeconds();
  ...
  //wk
  {
    ...
    if (user_intercation_->Get_value("speed", "", "Keep_speed") == 1.0 && 
        user_intercation_->Get_value("speed", "", "Flag") == 0.0){
        user_intercation_->Set_value("real", "speed", "", "Speed", injector_->vehicle_state()->linear_velocity());
      }
  }
  ...
  // envrionment_monitor();
  {
    std::lock_guard<std::mutex> lock(user_intercation_mutex_);
    envrionment_monitor();
    user_intercation_->record();
    user_intercation_->Update_real();
    UpdateFLAGS();
  }

  if (!ChangeLane()) {
    AINFO << "Fail change lane by user interaction";
  }
  ...
  //wk
  // if (!Update_trajectory(ptr_trajectory_pb)){
  //   AERROR << "Update trajectory based on user intercation";
  // }
...
//wk
bool OnLanePlanning::ChangeLane(){
  if (user_intercation_->Get_value("trj", "", "change_lane") == 0.0){
   AINFO <<"///////////////////";
   return true;
  }

  if (FLAGS_is_change_lane != 0.0 || FLAGS_user_ask_change_lane == 1.0){
    return true;
  }

  //get user interaction
  double distance = (user_intercation_->Get_value("scenario", "change_lane", "Use_distance") > 0.0)?
                    user_intercation_->Get_value("scenario", "change_lane", "Use_distance") : 10.0;
  double change_lr = user_intercation_->Get_value("scenario", "change_lane", "l_r");
  //change_lr = num_text;
  double times = user_intercation_->Get_value("scenario", "change_lane", "times");
  //times = num_text;
  double change_fx = user_intercation_->Get_value("scenario", "change_lane", "g_l_r");
  double change_distance = user_intercation_->Get_value("scenario", "change_lane", "distance");
  //get location
  apollo::common::PointENU ego_local;
  ego_local.set_x(local_view_.localization_estimate->pose().position().x());
  ego_local.set_y(local_view_.localization_estimate->pose().position().y());
  hdmap::LaneInfoConstPtr lane;
  double s;
  double l;
  static constexpr double kSearchRadius = 3.0;
  static constexpr double kMaxHeadingDiff = 1.0;
  if (local_view_.localization_estimate->pose().has_heading()) {
    if (nullptr == hdmap_ ||
          hdmap_->GetNearestLaneWithHeading(ego_local, kSearchRadius, local_view_.localization_estimate->pose().heading(),
    ...
    if (nullptr == hdmap_ ||
         hdmap_->GetNearestLane(ego_local, &lane, &s, &l) < 0) {
       AERROR << "Failed to get nearest lane";
       return false;
     }
  }

  AINFO <<"x0: " << ego_local.x() << "y0: " << ego_local.y();

  //related value
  const Vec2d left_vec = Vec2d::CreateUnitVec2d(local_view_.localization_estimate->pose().heading() + M_PI / 2.0);
  const Vec2d front_vec = Vec2d::CreateUnitVec2d(local_view_.localization_estimate->pose().heading());
  double change_x;
  //get new location
  if (change_lr == 1.0){
    while (times > 0.0) {
      if (!lane->lane().left_neighbor_forward_lane_id().empty()){
        change_x = lane->GetWidth(s);
        //hdmap::Id new_lane_id = lane->lane().left_neighbor_forward_lane_id(0);
        //lane = hdmap_->GetLaneById(new_lane_id);
        Vec2d new_location(ego_local.x(), ego_local.y());
        new_location += left_vec * change_x;
        ego_local.set_x(new_location.x());
        ego_local.set_y(new_location.y());
        if (local_view_.localization_estimate->pose().has_heading()) {
            if (nullptr == hdmap_ ||
                  hdmap_->GetNearestLaneWithHeading(ego_local, kSearchRadius, local_view_.localization_estimate->pose().heading(),
                                                    kMaxHeadingDiff, &lane, &s, &l) < 0) {
  ...
  } else if (change_lr == 2.0) {
    while (times > 0.0) {
      if (!lane->lane().right_neighbor_forward_lane_id().empty()){
        change_x = lane->GetWidth(s);
        Vec2d new_location(ego_local.x(), ego_local.y());
        new_location -= left_vec * change_x;
        ego_local.set_x(new_location.x());
  ...
  if (change_fx == 1.0){
    while (lane->lane().direction() != apollo::hdmap::Lane::LEFT_TURN){
      if (!lane->lane().left_neighbor_forward_lane_id().empty()){
        change_x = lane->GetWidth(s);
        Vec2d new_location(ego_local.x(), ego_local.y());
        new_location += left_vec * change_x;
  ...
  } else if (change_fx == 2.0) {
    while (lane->lane().direction() != apollo::hdmap::Lane::RIGHT_TURN){
      if (!lane->lane().right_neighbor_forward_lane_id().empty()){
        change_x = lane->GetWidth(s);
        Vec2d new_location(ego_local.x(), ego_local.y());
        new_location -= left_vec * change_x;
  ...
  } else if (change_fx == 3.0) {
    while (lane->lane().direction() != apollo::hdmap::Lane::NO_TURN){
      if ((lane->lane().direction() == apollo::hdmap::Lane::LEFT_TURN || lane->lane().direction() == apollo::hdmap::Lane::U_TURN) && !lane->lane().right_neighbor_forward_lane_id().empty()){
        change_x = lane->GetWidth(s);
        Vec2d new_location(ego_local.x(), ego_local.y());
        new_location -= left_vec * change_x;
        ego_local.set_x(new_location.x());
        ...
        distance += distance;
      } else if (lane->lane().direction() == apollo::hdmap::Lane::RIGHT_TURN && !lane->lane().left_neighbor_forward_lane_id().empty()){
        change_x = lane->GetWidth(s);
        Vec2d new_location(ego_local.x(), ego_local.y());
        new_location += left_vec * change_x;
        ego_local.set_x(new_location.x());
  ...
  Vec2d new_location(ego_local.x(), ego_local.y());
  if (change_distance > 0.0) {
    distance = change_distance;
  }
  new_location += front_vec * distance;
  ego_local.set_x(new_location.x());
  ego_local.set_y(new_location.y());
  // if (local_view_.localization_estimate->pose().has_heading()) {
  //   if (nullptr == hdmap_ ||
  ...
  AINFO <<"x1: " << ego_local.x() << "y1: " << ego_local.y();
  if (!frame_->ChangeLane(injector_->planning_context(), ego_local.x(), ego_local.y())){
    AINFO <<"+++++++++++++++++++";
    return false;
  }
  //num_text = 0.0;

  FLAGS_user_ask_change_lane = 1.0;

  {
    std::lock_guard<std::mutex> lock(user_intercation_mutex_);
    if (!user_intercation_->Set_value("real", "scenario", "change_lane", "l_r", 0.0) ||
        !user_intercation_->Set_value("real", "scenario", "change_lane", "times", 0.0) ||
        !user_intercation_->Set_value("real", "scenario", "change_lane", "g_l_r", 0.0) ||
        !user_intercation_->Set_value("real", "scenario", "change_lane", "distance", 0.0) ||
        !user_intercation_->Set_value("real", "trj", "", "change_lane", 0.0)) {
        //AINFO << "---------------------";
        return false;
    }
  }
  AINFO << "distance: " << user_intercation_->Get_value("scenario", "change_lane", "distance");

  return true;
}

// bool OnLanePlanning::Update_trajectory(ADCTrajectory* const ptr_trajectory_pb){
//   if (user_intercation_->Get_value("speed", "", "Speed") == 0.0) {
//         return true;
//       }

//   if (user_intercation_->Get_value("speed", "", "Without_check") != 1.0){
//     if (user_intercation_->Get_value("speed", "", "Speed") > FLAGS_speed_upper_bound ||
//         user_intercation_->Get_value("speed", "", "Speed") < FLAGS_speed_lower_bound) {
//           AERROR << "Speed check fail!";
//           {
//             std::lock_guard<std::mutex> lock(user_intercation_mutex_);
//             user_intercation_->Set_value("real", "speed", "", "Speed", 0.0);
//             user_intercation_->Set_value("real", "speed", "", "Acceleration", 0.0);
//           }
//           return false;
//         }
//     if (user_intercation_->Get_value("speed", "", "Acceleration") > FLAGS_longitudinal_acceleration_upper_bound ||
//         user_intercation_->Get_value("speed", "", "Acceleration") < FLAGS_longitudinal_acceleration_lower_bound){
//           AERROR << "Acceleration check fail!";
//           {
//             std::lock_guard<std::mutex> lock(user_intercation_mutex_);
//             user_intercation_->Set_value("real", "speed", "", "Speed", 0.0);
//             user_intercation_->Set_value("real", "speed", "", "Acceleration", 0.0);
...
//   const double speed = user_intercation_->Get_value("speed", "", "Speed");
//   double acceleration = user_intercation_->Get_value("speed", "", "Acceleration");
//   const double keep_speed = user_intercation_->Get_value("speed", "", "Keep_speed");
//   AINFO << "wk:adc_speed" << adc_speed;
//   AINFO << "wk:speed" << speed;
  
//   if (speed > 0.0){
//     if (speed < adc_speed) {
//       acceleration = (acceleration == 0.0)? FLAGS_longitudinal_acceleration_lower_bound : acceleration;
//     } else if (speed > adc_speed){
//       acceleration = (acceleration == 0.0)? FLAGS_longitudinal_acceleration_upper_bound : acceleration;
//     } else {
//       acceleration = 0.0;
//     }
//   }
...
//     if (acceleration > 0.0){
//       if (adc_speed + acceleration * p.relative_time() < speed + 0.5){
//         p.set_a(acceleration);
//         p.set_v(adc_speed + acceleration * p.relative_time());
//       } else {
//         if (keep_speed == 1.0 || (keep_speed == 3.0 && p.v() < speed)){
//           p.set_a(0.0);
//           p.set_v(speed);
//         }
//       }
...
//         p.set_v(adc_speed + acceleration * p.relative_time());
//       } else {
//         if (keep_speed == 1.0 || (keep_speed == 2.0 && p.v() > speed)){
//           p.set_a(0.0);
//           p.set_v(speed);
//         }
//       }
//     } else {
//       if (keep_speed == 1.0 || (keep_speed == 2.0 && p.v() > speed) || (keep_speed == 3.0 && p.v() < speed)) {
//         p.set_a(0.0);
//         p.set_v(speed);
//       }
//     }
...
//   if (std::abs(speed - adc_speed) <= 0.5){
//     if (keep_speed >= 1.0) {
//       {
//         std::lock_guard<std::mutex> lock(user_intercation_mutex_);
//         user_intercation_->Set_value("real", "speed", "", "Acceleration", 0.0);
...
//         user_intercation_->Set_value("real", "speed", "", "Speed", 0.0);
//         user_intercation_->Set_value("real", "speed", "", "Acceleration", 0.0);
//         user_intercation_->Set_value("real", "speed", "", "Flag", 0.0);
//       }
//     }
//   }
//   return true;
...
void OnLanePlanning::envrionment_monitor(){
  //ego info
  user_intercation_->Set_yueshu("egospeed", injector_->vehicle_state()->linear_velocity());
  user_intercation_->Set_yueshu("egoacceleration", injector_->vehicle_state()->linear_acceleration());
  double is_lane = 0.0;
  apollo::common::PointENU ego_local;
  ego_local.set_x(local_view_.localization_estimate->pose().position().x());
  ego_local.set_y(local_view_.localization_estimate->pose().position().y());
  double heading = local_view_.localization_estimate->pose().heading();
  const Vec2d front_vec = Vec2d::CreateUnitVec2d(heading);
  const Vec2d back_vec = Vec2d::CreateUnitVec2d(heading + M_PI);
  auto Polygon_of_ego = GetPolygon(ego_local, heading, 4.7, 2.06, 2.697298);
  std::vector<LaneInfoConstPtr> lanes;
  if (hdmap_->GetLanes(ego_local, 1.0, &lanes)!=0){
    AINFO << "Module - Fail to get lanes from base_map.";
  }
  if (lanes.size() >= 1){
    is_lane = 1.0;
  }
  ...
  if (local_view_.localization_estimate->pose().has_heading()) {
    if (nullptr == hdmap_ ||
          hdmap_->GetNearestLaneWithHeading(ego_local, kSearchRadius, heading,
                                            kMaxHeadingDiff, &lane, &s, &l) < 0) {
          AINFO << "Failed to get nearest lane with heading";
        }
  } else {
    if (nullptr == hdmap_ ||
         hdmap_->GetNearestLane(ego_local, &lane, &s, &l) < 0) {
       AINFO << "Failed to get nearest lane";
     }
  }

  //road condition
  double cross_dis = std::numeric_limits<double>::max();
  double stop_dis = std::numeric_limits<double>::max();
  double yield_dis = std::numeric_limits<double>::max();
  double keepclear_dis = std::numeric_limits<double>::max();
  const auto routing_end = local_view_.end_lane_way_point;
  double des_dis = std::numeric_limits<double>::max();
  double intersection_dis = std::numeric_limits<double>::max();
  double interaction_s = std::numeric_limits<double>::max();
  double is_intercation = 0.0;
  double traffic_light_dis = std::numeric_limits<double>::max();
  double trafficlight_color = 0.0;
  double traffic_light_s = std::numeric_limits<double>::max();
  double whether_trafficlight = 0.0;
  for (auto& reference_line_info_ : *frame_->mutable_reference_line_info()){
    ...
    for (auto crosswalk_overlap : crosswalk_overlaps_) {
        if (crosswalk_overlap->start_s >= adc_front_edge_s){
          cross_dis = std::min(cross_dis, crosswalk_overlap->start_s - adc_front_edge_s);
        }
        
    }

    //stop
    const std::vector<hdmap::PathOverlap>& stop_sign_overlaps = reference_line_info->reference_line().map_path().stop_sign_overlaps();
    for (const auto& stop_sign_overlap : stop_sign_overlaps){
      if (stop_sign_overlap.start_s >= adc_front_edge_s) {
        stop_dis = std::min(stop_dis, stop_sign_overlap.start_s - adc_front_edge_s);
      }
    }

    //yield
    const std::vector<hdmap::PathOverlap>& yield_sign_overlaps = reference_line_info->reference_line().map_path().yield_sign_overlaps();
    for (const auto& yield_sign_overlap : yield_sign_overlaps) {
      if (yield_sign_overlap.start_s >= adc_front_edge_s) {
        yield_dis = std::min(yield_dis, yield_sign_overlap.start_s - adc_front_edge_s);
      }
    }

    //keepclear
    const std::vector<hdmap::PathOverlap>& keep_clear_overlaps = reference_line_info->reference_line().map_path().clear_area_overlaps();
    for (const auto& keep_clear_overlap : keep_clear_overlaps) {
      if (keep_clear_overlap.start_s >= adc_front_edge_s) {
        keepclear_dis = std::min(keepclear_dis, keep_clear_overlap.start_s - adc_front_edge_s);
      }
    }
    ...
    for (const auto& overlap : traffic_light_overlaps) {
      if (overlap.start_s >= adc_front_edge_s && overlap.start_s - adc_front_edge_s < traffic_light_dis){
        traffic_light_dis = overlap.start_s - adc_front_edge_s;
        const auto& signal_color = frame_->GetSignal(overlap.object_id).color();
        if (signal_color == perception::TrafficLight::GREEN){
          trafficlight_color = 1.0;
        } else if (signal_color == perception::TrafficLight::YELLOW){
          trafficlight_color = 2.0;
        } else if (signal_color == perception::TrafficLight::RED){
          trafficlight_color = 3.0;
        }
        traffic_light_s = overlap.start_s;
      }
    }

    //des
    if (!frame_->is_near_destination()){
      continue;
    }
    common::SLPoint dest_sl;
    const auto& reference_line = reference_line_info->reference_line();
    reference_line.XYToSL(routing_end->pose(), &dest_sl);
    if (dest_sl.s() >= adc_front_edge_s) {
      des_dis = std::min(des_dis, dest_sl.s() - adc_front_edge_s);
    }

  }
  user_intercation_->Set_yueshu("cross_dis", cross_dis);
  user_intercation_->Set_yueshu("stop_dis", stop_dis);
  user_intercation_->Set_yueshu("yield_dis", yield_dis);
  user_intercation_->Set_yueshu("keepclear_dis", keepclear_dis);
  std::string intersection_id;
  double interaction_zwlx= 0.0;
  std::vector<JunctionInfoConstPtr> ego_junctions;
  const auto& reference_line_info = frame_->reference_line_info().front();
  if (hdmap_->GetJunctions(ego_local, 200, &ego_junctions) != 0) {
      AINFO << "SY Module - Fail to get junctions from base_map.";
  }
  if (ego_junctions.size() >= 1){
    for (const auto ego_junction: ego_junctions){
      if (!CheckWhetherJunctionisAhead(ego_junction->junction().id(), heading, ego_local.x(), ego_local.y())){
        continue;
      }
      const auto shape = ego_junction->junction().polygon();
      double tmp1 = std::numeric_limits<double>::max();
      ...
      for (unsigned int aaa = 0; (unsigned int) aaa < Polygon_of_ego.size(); aaa++){
        ...
        for (int i=0; i < shape.point_size(); i++){
          ...
          if (i+1 < shape.point_size()) {
            ...
            next_point = shape.point(0);
          }
          tmp1 = std::min(tmp1, CalculationDistanceToALine(polygon_x,polygon_y,every_point.x(), every_point.y(), next_point.x(), next_point.y()));
          if (is_in_intersection == false){
            jiaodianshu = jiaodianshu + doRectangleAndPolygonIntersect(polygon_x, polygon_y, every_point.x(), every_point.y(), next_point.x(), next_point.y());
          }
        }
      ...
      if (tmp1 < intersection_dis){
        intersection_dis = tmp1;
        intersection_id = ego_junction->id().id();
      }

      if (is_in_intersection){
        is_intercation = 1.0;
        intersection_dis = 0.0;
        interaction_s = s;
        intersection_id = ego_junction->id().id();
      }
    }
  ...
  // if (intersection_dis == 0.0){
  //   is_intercation = 1.0;
  //   interaction_s = s;
  // }

  if (traffic_light_dis < std::numeric_limits<double>::max() && intersection_dis < std::numeric_limits<double>::max() && traffic_light_dis - intersection_dis <= 10.0){
    whether_trafficlight = 1.0;
  }

  if (traffic_light_s < std::numeric_limits<double>::max() || interaction_s < std::numeric_limits<double>::max()){
    const auto& turn_type = reference_line_info.GetPathTurnType(std::min(traffic_light_s, interaction_s));
    if (turn_type == hdmap::Lane::LEFT_TURN) {
      interaction_zwlx = 1.0;
    } else if (turn_type == hdmap::Lane::RIGHT_TURN) {
      interaction_zwlx = 2.0;
    }
  }

  double is_fast_lane = 0.0;
  if(lane->lane().left_neighbor_forward_lane_id().empty() && !lane->lane().right_neighbor_forward_lane_id().empty()){
    if (is_lane == 1.0){
      is_fast_lane = 1.0;
    }
  }
  user_intercation_->Set_yueshu("is_lane", is_lane); 
  user_intercation_->Set_yueshu("is_fast_lane", is_fast_lane);
  
  user_intercation_->Set_yueshu("intersection_dis", intersection_dis);
  user_intercation_->Set_yueshu("traffic_light_dis", traffic_light_dis);
  user_intercation_->Set_yueshu("whether_trafficlight", whether_trafficlight);
  user_intercation_->Set_yueshu("interaction_zwlx", interaction_zwlx);
  user_intercation_->Set_yueshu("is_intercation", is_intercation);
  user_intercation_->Set_yueshu("trafficlight_color", trafficlight_color);

  //state
  user_intercation_->Set_yueshu("is_change_lane", FLAGS_is_change_lane);
  user_intercation_->Set_yueshu("is_borrow_lane", FLAGS_is_borrow_lane);
  user_intercation_->Set_yueshu("is_e_stop", FLAGS_is_e_stop);
  user_intercation_->Set_yueshu("is_pull_over", FLAGS_is_pull_over);

  //obstacle
  double v_f_dis = std::numeric_limits<double>::max();
  double v_f_speed = 0.0;
  double v_b_dis = std::numeric_limits<double>::max();
  double v_b_speed = 0.0;
  double v_l_dis = std::numeric_limits<double>::max();
  double v_l_speed = 0.0;
  double v_r_dis = std::numeric_limits<double>::max();
  double v_r_speed = 0.0;
  double r_f_dis = std::numeric_limits<double>::max();
  double r_f_speed = 0.0;
  double r_b_dis = std::numeric_limits<double>::max();
  double r_b_speed = 0.0;
  double r_l_dis = std::numeric_limits<double>::max();
  double r_l_speed = 0.0;
  double r_r_dis = std::numeric_limits<double>::max();
  double r_r_speed = 0.0;
  double o_f_dis = std::numeric_limits<double>::max();
  double o_b_dis = std::numeric_limits<double>::max();
  double o_l_dis = std::numeric_limits<double>::max();
  double o_r_dis = std::numeric_limits<double>::max(); 
  double is_jam = 0.0;

  double jishu = 0.0;
  bool biaojijingzhi = false;
  for (int i=0; i < local_view_.prediction_obstacles->prediction_obstacle_size(); i++){
    ...
    apollo::common::PointENU obj_local;
    obj_local.set_x(every_obstacle.perception_obstacle().position().x());
    obj_local.set_y(every_obstacle.perception_obstacle().position().y());
    double obj_heading = every_obstacle.perception_obstacle().theta();
    const auto type_of_obstacle = every_obstacle.perception_obstacle().type();
    const Vec2d obj_front_vec = Vec2d::CreateUnitVec2d(obj_heading);
    double obj_speed = every_obstacle.perception_obstacle().velocity().x() * obj_front_vec.x() + every_obstacle.perception_obstacle().velocity().y() * obj_front_vec.y();
    std::vector<JunctionInfoConstPtr> junctions;
    std::vector<LaneInfoConstPtr> obj_lanes;
    bool in_same_lane = false;
    if (hdmap_->GetJunctions(obj_local, 1, &junctions) != 0) {
      AINFO << "SY Module - Fail to get junctions from base_map.";
    }
    if (junctions.size() >= 1){
      for (const auto junction: junctions){
        std::string id = junction->id().id();
    ...
    if (hdmap_->GetLanes(obj_local, 1, &obj_lanes) != 0) {
      AINFO << "SY Module - Fail to get lanes from base_map.";
    }
    if (obj_lanes.size() >= 1){
      for (const auto obj_lane : obj_lanes){
        std::string id = obj_lane->id().id();
        if (id == lane->lane().id().id()){
          in_same_lane = true;
        }
      }
    }

    double obj_length = every_obstacle.perception_obstacle().length();
    double obj_width = every_obstacle.perception_obstacle().width();
    auto Polygon_of_obstacle = GetPolygon(obj_local, obj_heading, obj_length, obj_width, 0.0);
    double obj_fangwi = front_vec.x() * (obj_local.y() - ego_local.y()) - front_vec.y() * (obj_local.x() - ego_local.x());
    //jisuanqianhou
    double AB_x = obj_local.x() - ego_local.x();
    double AB_y = obj_local.y() - ego_local.y();
    double AB_length = std::sqrt(AB_x * AB_x + AB_y * AB_y);
    double obj_fangwi2 = front_vec.x() * AB_x + front_vec.y() * AB_y;
    double obj_fangwi3 = back_vec.x() * AB_x + back_vec.y() * AB_y;
    double cos_theta2 = obj_fangwi2 / (AB_length);
    double cos_theta3 = obj_fangwi3 / (AB_length);
    double theta2 = std::acos(cos_theta2);
    double theta3 = std::acos(cos_theta3);
    double obj_dis = CalculateDistanceFromObstacleToVehicle(Polygon_of_ego, Polygon_of_obstacle);

    // check if it's front or back
    if ((theta2 <= M_PI / 12 && in_same_lane) || (is_intercation == 1.0)){
      if (type_of_obstacle == 5){
        if (obj_dis < v_f_dis){
          v_f_dis = obj_dis;
          v_f_speed = obj_speed;
        }
      } else if (type_of_obstacle == 3 || type_of_obstacle == 4){
        if (obj_dis < r_f_dis){
          r_f_dis = obj_dis;
          r_f_speed = obj_speed;
        }
      ...
      if (obj_speed == 0.0 && obj_dis < o_f_dis){
        o_f_dis = obj_dis;
      }
    } else if (theta3 <= M_PI / 4) {
      if (type_of_obstacle == 5){
        if (obj_dis < v_b_dis){
          v_b_dis = obj_dis;
          v_b_speed = obj_speed;
      ...
      if (obj_speed == 0.0 && obj_dis < o_b_dis){
        o_b_dis = obj_dis;
      }
    } 
    ...
    if (obj_fangwi > 0.0) {
      ...
      if (obj_speed == 0.0 && obj_dis < o_l_dis){
        o_l_dis = obj_dis;
      }
    } else if (obj_fangwi < 0.0) {
      if (type_of_obstacle == 5){
        if (obj_dis < v_r_dis){
          v_r_dis = obj_dis;
          v_r_speed = obj_speed;
      ...
      if (obj_speed == 0.0 && obj_dis < o_r_dis){
        o_r_dis = obj_dis;
      }
    }
  ...
  if (biaojijingzhi && jishu >= 6){
    is_jam = 1.0;
  }

  user_intercation_->Set_yueshu("v_f_dis", v_f_dis);
  user_intercation_->Set_yueshu("v_f_speed", v_f_speed);
  user_intercation_->Set_yueshu("v_b_dis", v_b_dis);
  user_intercation_->Set_yueshu("v_b_speed", v_b_speed);
  user_intercation_->Set_yueshu("v_l_dis", v_l_dis);
  user_intercation_->Set_yueshu("v_l_speed", v_l_speed);
  user_intercation_->Set_yueshu("v_r_dis", v_r_dis);
  user_intercation_->Set_yueshu("v_r_speed", v_r_speed);
  user_intercation_->Set_yueshu("r_f_dis", r_f_dis);
  user_intercation_->Set_yueshu("r_f_speed", r_f_speed);
  user_intercation_->Set_yueshu("r_b_dis", r_b_dis);
  user_intercation_->Set_yueshu("r_b_speed", r_b_speed);
  user_intercation_->Set_yueshu("r_l_dis", r_l_dis);
  user_intercation_->Set_yueshu("r_l_speed", r_l_speed);
  user_intercation_->Set_yueshu("r_r_dis", r_r_dis);
  user_intercation_->Set_yueshu("r_r_speed", r_r_speed);
  user_intercation_->Set_yueshu("o_f_dis", o_f_dis);
  user_intercation_->Set_yueshu("o_b_dis", o_b_dis);
  user_intercation_->Set_yueshu("o_l_dis", o_l_dis);
  user_intercation_->Set_yueshu("o_r_dis", o_r_dis);
  user_intercation_->Set_yueshu("is_jam", is_jam);

  //weather
  user_intercation_->Set_yueshu("rain", user_intercation_->Get_weather("rain"));
  user_intercation_->Set_yueshu("rain", user_intercation_->Get_weather("snow"));
  user_intercation_->Set_yueshu("rain", user_intercation_->Get_weather("fog"));
  user_intercation_->Set_yueshu("rain", user_intercation_->Get_weather("cloud"));
  return;
}

std::vector<apollo::common::PointENU> OnLanePlanning::GetPolygon(apollo::common::PointENU original_point, double heading, 
                                                    double length, double width, double zhouju){
  std::vector<apollo::common::PointENU> result;
  apollo::common::PointENU point0;
  double x = original_point.x() + (length - zhouju)/2 + zhouju;
  double y = original_point.y()+ width/2;
  point0 = AddHeadingToEgo(heading, x, y, original_point.x(),original_point.y());
  result.push_back(point0);
  apollo::common::PointENU point1;
  x = original_point.x() + (length - zhouju)/2 + zhouju;
  y = original_point.y() - width/2;
  point1 = AddHeadingToEgo(heading, x, y, original_point.x(), original_point.y());
  result.push_back(point1);
  apollo::common::PointENU point2;
  x = original_point.x() - (length - zhouju)/2 ;
  y = original_point.y() - width/2;
  point2 = AddHeadingToEgo(heading, x, y, original_point.x(), original_point.y());
  result.push_back(point2);
  apollo::common::PointENU point3;
  x = original_point.x() - (length - zhouju)/2;
  y = original_point.y() + width/2;
  point3 = AddHeadingToEgo(heading, x, y, original_point.x(), original_point.y());
  result.push_back(point3);
  return result; 
}

apollo::common::PointENU OnLanePlanning::AddHeadingToEgo(double angle, double x, double y, double pointx, double pointy){
...
double OnLanePlanning::CalculateDistanceFromObstacleToVehicle(std::vector<apollo::common::PointENU> ego, std::vector<apollo::common::PointENU> obj){
  double dis = std::numeric_limits<double>::max();
  for (unsigned int aaa = 0; (unsigned int) aaa < ego.size(); aaa++ ){
    double polygon_x = ego[aaa].x();
    double polygon_y = ego[aaa].y();
    dis = std::min(dis, CalculationDistanceToALine(polygon_x, polygon_y, obj[0].x(), obj[0].y(), obj[1].x(), obj[1].y()));
    dis = std::min(dis, CalculationDistanceToALine(polygon_x, polygon_y, obj[1].x(), obj[1].y(), obj[2].x(), obj[2].y()));
    dis = std::min(dis, CalculationDistanceToALine(polygon_x, polygon_y, obj[2].x(), obj[2].y(), obj[3].x(), obj[3].y()));
    dis = std::min(dis, CalculationDistanceToALine(polygon_x, polygon_y, obj[3].x(), obj[3].y(), obj[0].x(), obj[0].y()));
  }
  return dis;
}

double OnLanePlanning::CalculationDistanceToALine(double x, double y, double start_x, double start_y, double end_x, double end_y){
  double E_F = x;
  double E_S = y;
  double A_F = start_x;
  double A_S = start_y;
...
bool OnLanePlanning::CheckWhetherJunctionisAhead(apollo::hdmap::Id the_id, double ego_heading, double x, double y){
  const auto shape = hdmap_->GetJunctionById(the_id)->junction().polygon();
  auto point = shape.point(0);
  const Vec2d front_vec = Vec2d::CreateUnitVec2d(ego_heading);
  double result = (point.x() - x) * front_vec.x() + (point.y() - y) * front_vec.y();
  if (result > 0) {
    return true;
  } else {
    return false;
...
//wk
void OnLanePlanning::UpdateFLAGS(){
  FLAGS_planning_upper_speed_limit = (user_intercation_->Get_value("preference", "", "Max_planning_speed") > 0.0)?
                                        user_intercation_->Get_value("preference", "", "Max_planning_speed") :
                                        FLAGS_planning_upper_speed_limit;
  //FLAGS_slowdown_profile_deceleration = (user_intercation_->Get_value("preference", "", "slowdown_profile_deceleration") > 0.0)?
  //                                      user_intercation_->Get_value("preference", "", "slowdown_profile_deceleration") :
  //                                      FLAGS_slowdown_profile_deceleration;
  FLAGS_near_stop_speed = (user_intercation_->Get_value("preference", "", "Near_stop_speed") > 0.0)?
                                        user_intercation_->Get_value("preference", "", "Near_stop_speed") :
                                        FLAGS_near_stop_speed;
  FLAGS_near_stop_deceleration = (user_intercation_->Get_value("preference", "", "Near_stop_deceleration") > 0.0)?
                                        user_intercation_->Get_value("preference", "", "Near_stop_deceleration") :
                                        FLAGS_near_stop_deceleration;
  FLAGS_default_cruise_speed = (user_intercation_->Get_value("preference", "", "Default_cruise_speed") > 0.0)?
                                        user_intercation_->Get_value("preference", "", "Default_cruise_speed") :
                                        FLAGS_default_cruise_speed;
  // AINFO << "Here:" << FLAGS_default_cruise_speed;
  
  //side
  FLAGS_side = (user_intercation_->Get_value("preference", "", "Driving_side") > 0.0)?
                                        user_intercation_->Get_value("preference", "", "Driving_side") :
                                        FLAGS_side;

  //change lane
  FLAGS_prioritize_change_lane = (user_intercation_->Get_value("preference", "", "Whether_priority_change_lane") > 0.0)? 
                                  ((user_intercation_->Get_value("preference", "", "Whether_priority_change_lane") == 1.0)? 1 : 0) :
                                  FLAGS_prioritize_change_lane;
  // AINFO << "Here:::" << FLAGS_prioritize_change_lane;
  // FLAGS_change_lane_min_length = (user_intercation_->Get_value("scenario", "change_lane", "Use_distance") > 0.0)?
  //                                       user_intercation_->Get_value("scenario", "change_lane", "Use_distance") :
  //                                       FLAGS_change_lane_min_length;
  

  //security check
  FLAGS_speed_lower_bound = (user_intercation_->Get_value("constraint", "", "Trajectory_min_speed") > 0.0)?
                                        user_intercation_->Get_value("constraint", "", "Trajectory_min_speed") :
                                        FLAGS_speed_lower_bound;
  FLAGS_speed_upper_bound = (user_intercation_->Get_value("constraint", "", "Trajectory_max_speed") > 0.0)?
                                        user_intercation_->Get_value("constraint", "", "Trajectory_max_speed") :
                                        FLAGS_speed_upper_bound;
  FLAGS_longitudinal_acceleration_lower_bound = (user_intercation_->Get_value("constraint", "", "Trajectory_min_longitudinal_acceleration") > 0.0)?
                                        user_intercation_->Get_value("constraint", "", "Trajectory_min_longitudinal_acceleration") :
                                        FLAGS_longitudinal_acceleration_lower_bound;
  FLAGS_longitudinal_acceleration_upper_bound = (user_intercation_->Get_value("constraint", "", "Trajectory_max_longitudinal_acceleration") > 0.0)?
                                        user_intercation_->Get_value("constraint", "", "Trajectory_max_longitudinal_acceleration") :
                                        FLAGS_longitudinal_acceleration_upper_bound;
  FLAGS_lateral_acceleration_bound = (user_intercation_->Get_value("constraint", "", "Trajectory_max_lateral_acceleration") > 0.0)?
                                        user_intercation_->Get_value("constraint", "", "Trajectory_max_lateral_acceleration") :
                                        FLAGS_lateral_acceleration_bound;
  FLAGS_enable_trajectory_check = (user_intercation_->Get_value("constraint", "", "Whether_trajectory_check") > 0.0)?
                                        ((user_intercation_->Get_value("constraint", "", "Whether_trajectory_check") == 1.0)?
                                        1 : 0) : FLAGS_enable_trajectory_check;
  // AINFO << "Here:::" << FLAGS_enable_trajectory_check;
  //obstacle
  FLAGS_obstacle_lat_buffer = (user_intercation_->Get_value("obstacle", "", "Lateral_buffer_distance") > 0.0)?
                              user_intercation_->Get_value("obstacle", "", "Lateral_buffer_distance") :
                              FLAGS_obstacle_lat_buffer;
  FLAGS_obstacle_lon_start_buffer = (user_intercation_->Get_value("obstacle", "", "Forward_buffer_distance") > 0.0)?
                              user_intercation_->Get_value("obstacle", "", "Forward_buffer_distance") :
                              FLAGS_obstacle_lon_start_buffer;
  FLAGS_obstacle_lon_end_buffer = (user_intercation_->Get_value("obstacle", "", "Backward_buffer_distance") > 0.0)?
                              user_intercation_->Get_value("obstacle", "", "Backward_buffer_distance") :
                              FLAGS_obstacle_lon_end_buffer;
  FLAGS_follow_min_distance = (user_intercation_->Get_value("obstacle", "", "Dynamic_Obstacle_Follow_distance") > 0.0)?
                              user_intercation_->Get_value("obstacle", "", "Dynamic_Obstacle_Follow_distance") :
                              FLAGS_follow_min_distance;
  FLAGS_yield_distance = (user_intercation_->Get_value("obstacle", "", "Dynamic_Obstacle_Yield_distance") > 0.0)?
                              user_intercation_->Get_value("obstacle", "", "Dynamic_Obstacle_Yield_distance") :
                              FLAGS_yield_distance;
  FLAGS_min_stop_distance_obstacle = (user_intercation_->Get_value("obstacle", "", "Min_stop_distance") > 0.0)?
                              user_intercation_->Get_value("obstacle", "", "Min_stop_distance") :
                              FLAGS_min_stop_distance_obstacle;

  //wait
  FLAGS_wait = user_intercation_->Get_value("trj", "", "wait");

  //test
  //FLAGS_default_cruise_speed = 1.0;
}


int OnLanePlanning::doRectangleAndPolygonIntersect(double x, double y, double start_x, double start_y, double end_x, double end_y){
  double x1;
  if (((start_y < y && end_y >=y) || (start_y >= y && end_y < y)) && (start_x <= x || end_x <= x)){
    x1 = start_x + (y - start_y) * (end_x - start_x) / (end_y - start_y);
    if (x1 <= x){
      return 1;
    }
  }
