...
bool Rerouting::Init(const std::string& name, const std::shared_ptr<DependencyInjector>& injector) {
...
bool Rerouting::ChangeLaneFailRerouting() {
  ...
  for (const auto& ref_line_info : frame_->reference_line_info()) {
    if (ref_line_info.ReachedDestination() || ref_line_info.SDistanceToDestination() < kRerouteThresholdToEnd) {
      return true;
    }
  }
  const auto& segments = reference_line_info_->Lanes();
  //wk
  if (frame_->GetUserintercation()->Get_value("trj", "", "rerouting") == 0.0) {
  ...
  //wk
  {
    ...
    frame_->GetUserintercation()->Set_value("real", "trj", "", "rerouting", 0.0);
  }

  double current_time = Clock::NowInSeconds();

  //wk
  const double cooldown_time = (frame_->GetUserintercation()->Get_value("preference", "","Rerouting_wait_time") > 0.0)?
                              frame_->GetUserintercation()->Get_value("preference", "", "Rerouting_wait_time") :
                              config_.cooldown_time();

  if (rerouting->has_last_rerouting_time() && (current_time - rerouting->last_rerouting_time() < cooldown_time)) {
