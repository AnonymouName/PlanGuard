...
bool TrafficLight::Init(const std::string& name, const std::shared_ptr<DependencyInjector>& injector) {
...
void TrafficLight::MakeDecisions(Frame* const frame, ReferenceLineInfo* const reference_line_info) {
  ...
  //wk
  if ((!config_.enabled() && frame->GetUserintercation()->Get_value("scenario", "traffic_light_signal", "Whether_check") == 0.0)||
      frame->GetUserintercation()->Get_value("scenario", "traffic_light_signal", "Whether_check") == 2.0) {
    return;
  }

  const auto& traffic_light_status = injector_->planning_context()->planning_status().traffic_light();

  const double adc_front_edge_s = reference_line_info->AdcSlBoundary().end_s();
  const double adc_back_edge_s = reference_line_info->AdcSlBoundary().start_s();

  // debug info
  planning_internal::SignalLightDebug* signal_light_debug = reference_line_info->mutable_debug()->mutable_planning_data()->mutable_signal_light();
  ...
  const std::vector<PathOverlap>& traffic_light_overlaps = reference_line_info->reference_line().map_path().signal_overlaps();
  for (const auto& traffic_light_overlap : traffic_light_overlaps) {
    ...
    for (const auto& done_traffic_light_overlap_id : traffic_light_status.done_traffic_light_overlap_id()) {
    ...
    common::math::Vec2d adc_position = {injector_->vehicle_state()->x(), injector_->vehicle_state()->y()};
    ...
    const double stop_deceleration = util::GetADCStopDeceleration(injector_->vehicle_state(), adc_front_edge_s, traffic_light_overlap.start_s);
    ...
    // debug info
    planning_internal::SignalLightDebug::SignalDebug* signal_debug = signal_light_debug->add_signal();
    ...
    //wk
    if (stop_deceleration > -FLAGS_longitudinal_acceleration_lower_bound){//config_.max_stop_deceleration()) {
    ...
    //wk
    const double stop_distance = (frame->GetUserintercation()->Get_value("scenario", "traffic_light_signal", "Stopping_distance") > 0.0)?
                                frame->GetUserintercation()->Get_value("scenario", "traffic_light_signal", "Stopping_distance") :
