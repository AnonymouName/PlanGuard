...
void StopSign::MakeDecisions(Frame* const frame, ReferenceLineInfo* const reference_line_info) {
  ...
  if ((!config_.enabled() && 
      frame->GetUserintercation()->Get_value("scenario", "stop_signal", "Whether_check") == 0.0) || 
      frame->GetUserintercation()->Get_value("scenario", "stop_signal", "Whether_check") == 2.0) {
    return;
  }
  ...
  for (const auto& stop_sign_overlap : stop_sign_overlaps) {
    ...
    const std::vector<std::string> wait_for_obstacle_ids(stop_sign_status.wait_for_obstacle_id().begin(), stop_sign_status.wait_for_obstacle_id().end());
