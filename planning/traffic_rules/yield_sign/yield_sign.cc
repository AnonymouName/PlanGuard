...
void YieldSign::MakeDecisions(Frame* const frame, ReferenceLineInfo* const reference_line_info) {
  ...
  //wk
  if ((!config_.enabled() && frame->GetUserintercation()->Get_value("scenario", "yield_signal", "Whether_check") == 0.0) ||
      frame->GetUserintercation()->Get_value("scenario", "yield_signal", "Whether_check") == 2.0) {
    return;
  }
  ...
  const std::vector<PathOverlap>& yield_sign_overlaps = reference_line_info->reference_line().map_path().yield_sign_overlaps();
  for (const auto& yield_sign_overlap : yield_sign_overlaps) {
    ...
    const std::vector<std::string> wait_for_obstacle_ids(yield_sign_status.wait_for_obstacle_id().begin(), yield_sign_status.wait_for_obstacle_id().end());
