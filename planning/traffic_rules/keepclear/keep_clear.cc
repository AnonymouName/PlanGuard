...
Status KeepClear::ApplyRule(Frame* const frame, ReferenceLineInfo* const reference_line_info) {
  ...
  // keep_clear zone
  //wk
  if ((config_.enable_keep_clear_zone() && frame->GetUserintercation()->Get_value("scenario", "keep_clear_signal", "Whether_check") == 0.0) ||
    frame->GetUserintercation()->Get_value("scenario", "keep_clear_signal", "Whether_check") == 1.0) {
    const std::vector<PathOverlap>& keep_clear_overlaps = reference_line_info->reference_line().map_path().clear_area_overlaps();
    for (const auto& keep_clear_overlap : keep_clear_overlaps) {
      ...
      if (BuildKeepClearObstacle(frame, reference_line_info, obstacle_id, keep_clear_overlap.start_s, keep_clear_overlap.end_s)) {
  ...
  // junction
  //wk
  if ((config_.enable_junction() && frame->GetUserintercation()->Get_value("scenario", "keep_clear_signal", "Whether_check") == 0.0) ||
      frame->GetUserintercation()->Get_value("scenario", "keep_clear_signal", "Whether_check") == 1.0) {
    ...
    if (pnc_junction_overlap != nullptr) {
      ...
      if (!IsCreeping(pnc_junction_overlap->start_s, adc_front_edge_s)) {
        ...
        if (traffic_light_overlap != nullptr &&
            std::fabs(pnc_junction_start_s - traffic_light_overlap->start_s) <= config_.align_with_traffic_sign_tolerance()) {
          ...
          pnc_junction_start_s = traffic_light_overlap->start_s;
                   ...
                   std::fabs(pnc_junction_start_s - stop_sign_overlap->start_s) <= config_.align_with_traffic_sign_tolerance()) {
          ADEBUG << "adjust pnc_junction_start_s[" << pnc_junction_start_s
          ...
          pnc_junction_start_s = stop_sign_overlap->start_s;
                   ...
                   std::fabs(pnc_junction_start_s - crosswalk_overlap->start_s) <= config_.align_with_traffic_sign_tolerance()) {
          ADEBUG << "adjust pnc_junction_start_s[" << pnc_junction_start_s
        ...
        if (BuildKeepClearObstacle(frame, reference_line_info, obstacle_id, pnc_junction_start_s, pnc_junction_overlap->end_s)) {
