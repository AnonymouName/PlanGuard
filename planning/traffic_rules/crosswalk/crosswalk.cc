...
Status Crosswalk::ApplyRule(Frame* const frame,
  ...
  //wk
  if (!FindCrosswalks(reference_line_info) || frame->GetUserintercation()->Get_value("scenario", "crosswalk_signal", "Whether_check") == 2.0) {
...
void Crosswalk::MakeDecisions(Frame* const frame,
                              ReferenceLineInfo* const reference_line_info) {
  //wk
  const double stop_timeout = (frame->GetUserintercation()->Get_value("scenario", "crosswalk_signal", "Wait_time") > 0.0)?
                              frame->GetUserintercation()->Get_value("scenario", "crosswalk_signal", "Wait_time") :
                              config_.stop_timeout();
  const double pre_stop_distance = (frame->GetUserintercation()->Get_value("scenario", "crosswalk_signal", "Stopping_distance") > 0.0)?
                              frame->GetUserintercation()->Get_value("scenario", "crosswalk_signal", "Stopping_distance") :
  ...
  for (auto crosswalk_to_stop : crosswalks_to_stop) {
    ...
    util::BuildStopDecision(
        ...
        pre_stop_distance, StopReasonCode::STOP_REASON_CROSSWALK,
...
bool Crosswalk::CheckStopForObstacle(
  ...
  //wk
  if (stop) {
    if (stop_deceleration >= -FLAGS_longitudinal_acceleration_lower_bound) {
