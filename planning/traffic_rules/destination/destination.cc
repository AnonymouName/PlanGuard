...
int Destination::MakeDecisions(Frame* frame, ReferenceLineInfo* const reference_line_info) {
  ...
  //wk
  const double pre_stop_distance = (frame->GetUserintercation()->Get_value("scenario", "", "Stopping_distance") > 0.0)?
                                   frame->GetUserintercation()->Get_value("scenario", "", "Stopping_distance") :
  ...
  if (pull_over_status.has_position() && pull_over_status.position().has_x() && pull_over_status.position().has_y()) {
    ...
    //wk
    const double stop_line_s = pull_over_sl.s() + VehicleConfigHelper::GetConfig().vehicle_param().front_edge_to_center() + 
                               pre_stop_distance;
                               //config_.stop_distance();
    //wk
    util::BuildStopDecision(stop_wall_id, stop_line_s, pre_stop_distance, //config_.stop_distance(),
  ...
  //wk
  const double dest_lane_s = std::fmax(0.0, routing_end->s() - FLAGS_virtual_stop_wall_length - pre_stop_distance);
                         //config_.stop_distance());
  //wk
  util::BuildStopDecision(
      stop_wall_id, routing_end->id(), dest_lane_s, pre_stop_distance, //config_.stop_distance(),
