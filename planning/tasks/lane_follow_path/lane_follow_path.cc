...
bool LaneFollowPath::DecidePathBounds(std::vector<PathBoundary>* boundary) {
  ...
  if (is_include_adc) {
    PathBoundsDeciderUtil::ExtendBoundaryByADC(init_sl_state_, config_.extend_buffer(), &path_bound);
  }
  ...
  for (const auto* obs : indexed_obstacles.Items()) {
    ...
    for (int i = 0; i < sl_bound.boundary_point_size(); i++) {
      ...
      print_curve.AddPoint(name, sl_bound.boundary_point(i).s() - path_bound.start_s(),
