...
bool PullOverPath::DecidePathBounds(std::vector<PathBoundary>* boundary) {
  ...
  //wk
  const double stop_side = frame_->GetUserintercation()->Get_value("trj", "", "stop_side");
  if ((config_.pull_over_direction() == PullOverPathConfig::BOTH_SIDE && stop_side == 0.0) ||
      stop_side == 3.0) {
  ...
  } else if ((config_.pull_over_direction() == PullOverPathConfig::LEFT_SIDE && stop_side == 0.0) ||
              stop_side == 1.0) {
