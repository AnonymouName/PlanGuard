...
bool SpeedDecider::CheckKeepClearCrossable(
  ...
  //wk
  const double keep_clear_speed = (frame_->GetUserintercation()->Get_value("scenario", "keep_clear_signal", "Min_speed") > 0.0)?
                                  frame_->GetUserintercation()->Get_value("scenario", "keep_clear_signal", "Min_speed") > 0.0 :
                                  config_.keep_clear_last_point_speed();
  if (last_speed_point.s() <= keep_clear_st_boundary.max_s() &&
      last_speed_point_v < keep_clear_speed) {
...
bool SpeedDecider::IsFollowTooClose(const Obstacle& obstacle) const {
  ...
  //wk
  const double distance =
      obstacle.path_st_boundary().min_s() - FLAGS_follow_min_distance;//FLAGS_min_stop_distance_obstacle;
  static const double lane_follow_max_decel = -FLAGS_longitudinal_acceleration_lower_bound;//3.0;
  static const double lane_change_max_decel = -FLAGS_longitudinal_acceleration_lower_bound;//3.0;
...
bool SpeedDecider::CreateYieldDecision(
  ...
  PerceptionObstacle::Type obstacle_type = obstacle.Perception().type();
  //wk
  double yield_distance = FLAGS_yield_distance; //config_.yield_distance_buffer();
...
double SpeedDecider::EstimateProperOvertakingGap(const double target_obs_speed,
  ...
  //wk
  const double overtake_min_distance = (frame_->GetUserintercation()->Get_value("obstacle", "", "Dynamic_Obstacle_Overtake_distance") > 0.0)?
                                      frame_->GetUserintercation()->Get_value("obstacle", "", "Dynamic_Obstacle_Overtake_distance") :
                                      config_.overtake_min_distance();
  const double overtake_distance_s = std::fmax(
      std::fmax(adc_speed, target_obs_speed) * config_.overtake_time_buffer(), overtake_min_distance);
      //config_.overtake_min_distance());
