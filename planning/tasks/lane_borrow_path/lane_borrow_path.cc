...
apollo::common::Status LaneBorrowPath::Process(Frame* frame, ReferenceLineInfo* reference_line_info) {
  //wk
  if (frame->GetUserintercation()->Get_value("preference", "", "Whether_allow_borrow_lane") == 2.0 ||
      frame_->GetUserintercation()->Get_value("trj", "", "lane_follow") == 1.0){
    ADEBUG << "path reusable" << reference_line_info->path_reusable() << ",skip";
    FLAGS_is_borrow_lane = 0.0;
    return Status::OK();
  } else if (frame->GetUserintercation()->Get_value("preference", "", "Whether_allow_borrow_lane") == 0.0) {
  ...
  if (!IsNecessaryToBorrowLane()) {
    if (frame_->GetUserintercation()->Get_value("speed", "", "Flag") == 2.0){
      {
        std::lock_guard<std::mutex> lock(user_intercation_mutex_);
        frame_->GetUserintercation()->Set_value("real", "speed", "", "Speed", 0.0);
        frame_->GetUserintercation()->Set_value("real", "speed", "", "Keep_speed", 0.0);
        frame_->GetUserintercation()->Set_value("real", "speed", "", "Flag", 0.0);
      }
    }
    FLAGS_is_borrow_lane = 0.0;
  ...
  if (AssessPath(&candidate_path_data, reference_line_info->mutable_path_data())) {
    ...
    //wk
    FLAGS_is_borrow_lane = 0.0;
    const double borrowlane_speed = frame_->GetUserintercation()->Get_value("scenario", "borrow_lane", "Expected_speed");
    if (borrowlane_speed > 0.0 && frame_->GetUserintercation()->Get_value("real", "speed", "", "Flag") != 3.0) {
      {
        std::lock_guard<std::mutex> lock(user_intercation_mutex_);
        frame_->GetUserintercation()->Set_value("real", "speed", "", "Speed", borrowlane_speed);
        frame_->GetUserintercation()->Set_value("real", "speed", "", "Keep_speed", 1.0);
        frame_->GetUserintercation()->Set_value("real", "speed", "", "Flag", 2.0);
      }
    }
    {
      std::lock_guard<std::mutex> lock(user_intercation_mutex_);
      frame_->GetUserintercation()->Set_value("real", "trj", "", "borrow_lane", 0.0);
      frame_->GetUserintercation()->Set_value("real", "scenario", "borrow_lane", "l_r", 0.0);
    }
  } else {
    if (frame_->GetUserintercation()->Get_value("speed", "", "Flag") == 2.0){
      {
...
bool LaneBorrowPath::DecidePathBounds(std::vector<PathBoundary>* boundary) {
  for (size_t i = 0; i < decided_side_pass_direction_.size(); i++) {
    ...
    //wk
    const double if_check = frame_->GetUserintercation()->Get_value("scenario", "borrow_lane", "Whether_environment_check");
...
bool LaneBorrowPath::IsNecessaryToBorrowLane() {
  ...
  if (mutable_path_decider_status->is_in_path_lane_borrow_scenario()) {
    ...
    //wk
    if (!IsWithinSidePassingSpeedADC(*frame_) && frame_->GetUserintercation()->Get_value("scenario", "borrow_lane", "Whether_speed_check") != 2.0) {
      return false;
    }
    ...
    if (frame_->GetUserintercation()->Get_value("trj", "", "borrow_lane") == 1.0){
      decided_side_pass_direction_.clear();
    }
    ...
    if (decided_side_pass_direction_.empty()) {
      ...
      if (!left_borrowable && !right_borrowable) {
        ...
        //wk
        if (left_borrowable && frame_->GetUserintercation()->Get_value("scenario", "borrow_lane", "l_r") != 2.0) {
          decided_side_pass_direction_.push_back(SidePassDirection::LEFT_BORROW);
          mutable_path_decider_status->set_is_in_path_lane_borrow_scenario(true);
        }
        if (right_borrowable && frame_->GetUserintercation()->Get_value("scenario", "borrow_lane", "l_r") != 1.0) {
...
bool LaneBorrowPath::IsWithinSidePassingSpeedADC(const Frame& frame) {
  //wk
  const double check_speed = (frame_->GetUserintercation()->Get_value("scenario", "borrow_lane", "Expected_speed") > 0.0)? 
                            frame_->GetUserintercation()->Get_value("scenario", "borrow_lane", "Expected_speed") : 
                            config_.lane_borrow_max_speed();
  return frame.PlanningStartPoint().v() < check_speed;
}
...
bool LaneBorrowPath::IsLongTermBlockingObstacle() {
  ...
  if (injector_->planning_context()
      ...
      frame_->GetUserintercation()->Get_value("trj", "", "borrow_lane") == 1.0) {
...
bool LaneBorrowPath::IsBlockingObstacleWithinDestination(const ReferenceLineInfo& reference_line_info) {
  ...
  if (blocking_obstacle_id.empty() ||
      frame_->GetUserintercation()->Get_value("scenario", "borrow_lane", "Whether_environment_check") == 2.0) {
...
bool LaneBorrowPath::IsSidePassableObstacle(const ReferenceLineInfo& reference_line_info) {
  //wk
  if (frame_->GetUserintercation()->Get_value("scenario", "borrow_lane", "Whether_environment_check") == 2.0){
    return true;
  }
...
void LaneBorrowPath::CheckLaneBorrow(const ReferenceLineInfo& reference_line_info,
  ...
  *right_neighbor_lane_borrowable = true;
  ...
  //wk
  const double kLookforwardDistance = (frame_->GetUserintercation()->Get_value("scenario", "borrow_lane", "Check_distance") > 0.0)? 
                                                frame_->GetUserintercation()->Get_value("scenario", "borrow_lane", "Check_distance") : 
                                                100.0;
