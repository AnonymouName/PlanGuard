...
GriddedPathTimeGraph::GriddedPathTimeGraph(
  ...
  //wk
  max_acceleration_ =
      std::min(std::abs(vehicle_param_.max_acceleration()), FLAGS_longitudinal_acceleration_upper_bound);

              // std::abs(gridded_path_time_graph_config_.max_acceleration()));
  max_deceleration_ =
      -1.0 *
      std::min(std::abs(vehicle_param_.max_deceleration()), -FLAGS_longitudinal_acceleration_lower_bound);
