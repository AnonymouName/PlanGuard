...
Status SpeedLimitDecider::GetSpeedLimits(
  ...
  for (uint32_t i = 0; i < discretized_path.size(); ++i) {
    ...
    for (const auto* ptr_obstacle : obstacles.Items()) {
      ...
      if (is_close_on_left || is_close_on_right) {
        ...
        if (ptr_obstacle->IsStatic()) {
          //wk
          nudge_speed_ratio = (frame_->GetUserintercation()->Get_value("obstacle", "", "Static_Obstacle_Deceleration_ratio") > 0.0)? 
              frame_->GetUserintercation()->Get_value("obstacle", "", "Static_Obstacle_Deceleration_ratio") :
              speed_bounds_config_.static_obs_nudge_speed_ratio();
        } else {
          //wk
          nudge_speed_ratio = (frame_->GetUserintercation()->Get_value("obstacle", "", "Dynamic_Obstacle_Deceleration_ratio") > 0.0)? 
              frame_->GetUserintercation()->Get_value("obstacle", "", "Dynamic_Obstacle_Deceleration_ratio") :
   ...
   //wk
    const double enable_nudge_slowdown = frame_->GetUserintercation()->Get_value("obstacle", "", "Whether_declearation");

    if ((speed_bounds_config_.enable_nudge_slowdown() &&
        enable_nudge_slowdown == 0.0) ||
        enable_nudge_slowdown == 1.0) {
      curr_speed_limit =
