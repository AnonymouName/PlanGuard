...
apollo::common::Status LaneChangePath::Process(Frame* frame, ReferenceLineInfo* reference_line_info) {
  ...
  if (status != ChangeLaneStatus::IN_CHANGE_LANE) {
    if ( frame->GetUserintercation()->Get_value("speed", "", "Flag") == 1.0) {
      {
        std::lock_guard<std::mutex> lock(user_intercation_mutex_);
        frame->GetUserintercation()->Set_value("real", "speed", "", "Speed", 0.0);
        frame->GetUserintercation()->Set_value("real", "speed", "", "Keep_speed", 0.0);
        frame->GetUserintercation()->Set_value("real", "speed", "", "Flag", 0.0);
      }
    }
  ...
  //wk
  const double changelane_speed = frame->GetUserintercation()->Get_value("scenario", "change_lane", "Expected_speed");
  if (changelane_speed > 0.0 && frame->GetUserintercation()->Get_value("real", "speed", "", "Flag") != 3.0) {
    {
      std::lock_guard<std::mutex> lock(user_intercation_mutex_);
      frame->GetUserintercation()->Set_value("real", "speed", "", "Speed", changelane_speed);
      frame->GetUserintercation()->Set_value("real", "speed", "", "Keep_speed", 1.0);
      frame->GetUserintercation()->Set_value("real", "speed", "", "Flag", 1.0);
    }
  }
  return Status::OK();
}
...
bool LaneChangePath::DecidePathBounds(std::vector<PathBoundary>* boundary) {
  ...
  if (!PathBoundsDeciderUtil::ExtendBoundaryByADC(init_sl_state_, config_.extend_adc_buffer(), &path_bound)) {
  ...
  //wk
  const double if_check = frame_->GetUserintercation()->Get_value("scenario", "change_lane", "Whether_environment_check");
  
  if (!PathBoundsDeciderUtil::GetBoundaryFromStaticObstacles(
          *reference_line_info_, init_sl_state_, &path_bound,
          &blocking_obstacle_id, if_check)) {
...
bool LaneChangePath::OptimizePath(const std::vector<PathBoundary>& path_boundaries, std::vector<PathData>* candidate_path_data) {
  ...
  for (const auto& path_boundary : path_boundaries) {
    ...
    const double jerk_bound = PathOptimizerUtil::EstimateJerkBoundary(std::fmax(init_sl_state_.first[1], 1.0));
    std::vector<double> ref_l(path_boundary_size, 0);
    std::vector<double> weight_ref_l(path_boundary_size, 0);

    bool res_opt = PathOptimizerUtil::OptimizePath(init_sl_state_, end_state, ref_l, weight_ref_l, path_boundary,
...
void LaneChangePath::UpdateLaneChangeStatus() {
  ...
  if (!has_change_lane) {
    if (prev_status->status() == ChangeLaneStatus::IN_CHANGE_LANE) {
      ...
      FLAGS_is_change_lane = 0.0;
      FLAGS_user_ask_change_lane = 0.0;
    }
    return;
  }
  //wk
  const double time = frame_->GetUserintercation()->Get_value("scenario", "change_lane", "Time_interval");
  double time1 = (time > 0.0)? time : config_.change_lane_success_freeze_time();
  // if (FLAGS_user_ask_change_lane ==1.0){
  //   time1 = time1 + FLAGS_more_time;
  // }
  ...
  if (reference_line_info_->IsChangeLanePath()) {
    ...
    if (!CheckLastFrameSucceed(history_frame)) {
      ...
      FLAGS_is_change_lane = 2.0;
      return;
    }
    //wk
    if (frame_->GetUserintercation()->Get_value("trj", "", "lane_follow") == 1.0){
      UpdateStatus(now, ChangeLaneStatus::CHANGE_LANE_FAILED, change_lane_id);
      is_exist_lane_change_start_position_ = false;
      FLAGS_is_change_lane = 2.0;
      return;
...
bool LaneChangePath::IsClearToChangeLane(ReferenceLineInfo* reference_line_info) {
  ...
  for (const auto* obstacle : reference_line_info->path_decision()->obstacles().Items()) {
    ...
    //wk
    kForwardSafeDistance = (frame_->GetUserintercation()->Get_value("scenario", "change_lane", "Check_distance") > 0.0)?
                            frame_->GetUserintercation()->Get_value("scenario", "change_lane", "Check_distance") > 0.0 :
...
void LaneChangePath::GetLaneChangeStartPoint(const ReferenceLine& reference_line, double adc_frenet_s, common::math::Vec2d* start_xy) {
  //wk
  const double  lane_change_prepare_length = (frame_->GetUserintercation()->Get_value("scenario", "change_lane", "Preparation_distance") > 0.0)?
                                             frame_->GetUserintercation()->Get_value("scenario", "change_lane", "Preparation_distance") :
                                            config_.lane_change_prepare_length();
  //wk
  double lane_change_start_s = lane_change_prepare_length + adc_frenet_s;
...
void LaneChangePath::GetBoundaryFromLaneChangeForbiddenZone(PathBoundary* const path_bound) {
  ...
  if (is_exist_lane_change_start_position_) {
    ...
    //wk
    lane_change_start_s = lane_change_prepare_length + init_sl_state_.first[0];
