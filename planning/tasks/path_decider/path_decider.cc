...
bool PathDecider::MakeObjectDecision(const PathData &path_data, const std::string &blocking_obstacle_id,
                                     PathDecision *const path_decision) {
...
bool PathDecider::MakeStaticObstacleDecision(const PathData &path_data, const std::string &blocking_obstacle_id,
    PathDecision *const path_decision) {
  ...
  for (const auto *obstacle : path_decision->obstacles().Items()) {
    ...
    if (curr_l - lateral_radius > sl_boundary.end_l() || curr_l + lateral_radius < sl_boundary.start_l()) {
    ...
    } else if (sl_boundary.end_l() >= curr_l - min_nudge_l && sl_boundary.start_l() <= curr_l + min_nudge_l) {
      ...
      *object_decision.mutable_stop() = GenerateObjectStopDecision(*obstacle);
      ...
      if (path_decision->MergeWithMainStop(
        ...
        ObjectDecisionType object_decision;
        object_decision.mutable_ignore();
        path_decision->AddLongitudinalDecision("PathDecider/not-nearest-stop", obstacle->Id(), object_decision);
      }
...
bool PathDecider::IgnoreBackwardObstacle(PathDecision *const path_decision) {
  ...
  for (const auto *obstacle : path_decision->obstacles().Items()) {
    ...
    if (obstacle->Obstacle::PerceptionSLBoundary().end_s() < adc_start_s) {
      ObjectDecisionType object_decision;
      object_decision.mutable_ignore();
      path_decision->AddLongitudinalDecision("PathDecider/ignore-backward-obstacle", obstacle->Id(), object_decision);
    }
  }
  return true;
}
