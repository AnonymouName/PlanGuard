...
void RuleBasedStopDecider::CheckLaneChangeUrgency(Frame *const frame) {
  for (auto &reference_line_info : *frame->mutable_reference_line_info()) {
    //wk
    const double distance = frame_->GetUserintercation()->Get_value("scenario", "change_lane", "Check_distance");
    ...
    if (reference_line_info.IsChangeLanePath()) {
      is_clear_to_change_lane_ = IsClearToChangeLane(&reference_line_info, distance, if_check);
...
void RuleBasedStopDecider::AddPathEndStop(Frame *const frame, ReferenceLineInfo *const reference_line_info) {
...
void RuleBasedStopDecider::StopOnSidePass(Frame *const frame, ReferenceLineInfo *const reference_line_info) {
  //wk
  const double distance = frame_->GetUserintercation()->Get_value("scenario", "change_lane", "Check_distance");
  const double if_check = frame_->GetUserintercation()->Get_value("scenario", "change_lane", "Whether_environment_check");
  ...
  if (!check_clear && CheckSidePassStop(path_data, *reference_line_info, &stop_s_on_pathdata)) {
    if ((!IsPerceptionBlocked(*reference_line_info, config_.search_beam_length(),
        ...
        IsClearToChangeLane(reference_line_info, distance, if_check))) {
      return;
    }
    if (!CheckADCStop(path_data, *reference_line_info, stop_s_on_pathdata)) {
      ...
      if (IsClearToChangeLane(reference_line_info, distance, if_check)) {
        check_clear = true;
      }
    }
  }
...
bool RuleBasedStopDecider::CheckSidePassStop(const PathData &path_data, const ReferenceLineInfo &reference_line_info, double *stop_s_on_pathdata) {
  const std::vector<std::tuple<double, PathData::PathPointType, double>> &path_point_decision_guide = path_data.path_point_decision_guide();
  ...
  for (const auto &point_guide : path_point_decision_guide) {
    if (last_path_point_type == PathData::PathPointType::IN_LANE &&
      ...
      *stop_s_on_pathdata = std::get<0>(point_guide);
      ...
      Vec2d shift_vec{ego_front_to_center * std::cos(ego_theta), ego_front_to_center * std::sin(ego_theta)};
...
bool RuleBasedStopDecider::BuildSidePassStopFence(const PathData &path_data, const double stop_s_on_pathdata,
...
bool RuleBasedStopDecider::CheckADCStop(const PathData &path_data, const ReferenceLineInfo &reference_line_info, const double stop_s_on_pathdata) {
  ...
  //wk
  if (adc_speed > FLAGS_near_stop_speed){               //config_.max_adc_stop_speed()) {
...
bool RuleBasedStopDecider::CheckClearDone(const ReferenceLineInfo &reference_line_info, const common::PathPoint &stop_point) {
  const double adc_front_edge_s = reference_line_info.AdcSlBoundary().end_s();
  ...
  reference_line_info.reference_line().GetLaneWidth((adc_front_edge_s + adc_back_edge_s) / 2.0, &lane_left_width, &lane_right_width);
