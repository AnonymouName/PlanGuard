...
bool PullOverScenario::IsTransferable(const Scenario* const other_scenario,
  ...
  if ((!FLAGS_enable_pull_over_at_destination && 
      frame.GetUserintercation()->Get_value("scenario", "reach_destination", "Whether_pull_over") == 0.0) ||
      frame.GetUserintercation()->Get_value("scenario", "reach_destination", "Whether_pull_over") == 2.0) {
    return false;
  }
  ...
  //wk
  const double pre_distance = (frame.GetUserintercation()->Get_value("scenario", "reach_destination", "Preparation_distance") > 0.0)?
                              frame.GetUserintercation()->Get_value("scenario", "reach_destination", "Preparation_distance") :
  ...
  bool pull_over_scenario =
      (frame.reference_line_info().size() == 1 &&  // NO, while changing lane
       ...
       adc_distance_to_dest <= pre_distance);
           //context_.scenario_config.start_pull_over_scenario_distance());
  // too close to destination + not found pull-over position
  if (pull_over_scenario) {
    //wk
    const double pre_stop_distance = (frame.GetUserintercation()->Get_value("scenario", "reach_destination", "Stopping_distance") > 0.0)?
                                      frame.GetUserintercation()->Get_value("scenario", "reach_destination", "Stopping_distance") :
                                      context_.scenario_config.max_distance_stop_search();
    if (adc_distance_to_dest < pre_stop_distance){
        //context_.scenario_config.max_distance_stop_search()) {
  ...
  if (pull_over_scenario) {
    {
      ...
      frame.GetUserintercation()-> Init_port("real", "speed");
    }
  }
