...
bool PullOverStageRetryApproachParking::CheckADCStop(const Frame& frame) {
  ...
  //wk
  const double stop_distance = (frame.GetUserintercation()->Get_value("scenario", "reach_destination", "Stopping_distance") > 0.0)?
                                frame.GetUserintercation()->Get_value("scenario", "reach_destination", "Stopping_distance") :
                                GetContextAs<PullOverContext>()->scenario_config.max_valid_stop_distance();
