...
StageResult LaneFollowStage::Process(const TrajectoryPoint& planning_start_point, Frame* frame) {
  ...
  //wk
  const double lane_follow_speed = frame->GetUserintercation()->Get_value("scenario", "lane_follow", "Expected_speed");
  ...
  if (FLAGS_user_ask_change_lane == 1.0){
    ...
    for (auto& reference_line_info : *frame->mutable_reference_line_info()){
      ...
      if (lane_follow_speed > 0.0) {
        reference_line_info.LimitCruiseSpeed(lane_follow_speed);
      } else {
  ...
  if (has_drivable_reference_line == false){
    for (auto& reference_line_info : *frame->mutable_reference_line_info()) {
      ...
      if (!result.HasError()) {
        ...
        if (reference_line_info.Cost() < kStraightForwardLineCost) {
          ...
          AINFO << "wk: change lane";
          has_drivable_reference_line = true;
          reference_line_info.SetDrivable(true);
        } else {
          reference_line_info.SetDrivable(false);
          AINFO << "\tlane change failed";
        }
      } else {
        reference_line_info.SetDrivable(false);
      }
...
StageResult LaneFollowStage::PlanOnReferenceLine(const TrajectoryPoint& planning_start_point, Frame* frame,
  ...
  for (const auto* obstacle : reference_line_info->path_decision()->obstacles().Items()) {
    if (obstacle->LongitudinalDecision().has_stop() && obstacle->LongitudinalDecision().stop().reason_code() ==
