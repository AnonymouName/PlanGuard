...
bool EmergencyStopScenario::IsTransferable(const Scenario* const other_scenario, const Frame& frame) {
  const auto& pad_msg_driving_action = frame.GetPadMsgDrivingAction();
  if (pad_msg_driving_action == PadMessage::STOP ||
      frame.GetUserintercation()->Get_value("trj", "", "emergency_stop") == 1.0) {
    {
...
ScenarioResult EmergencyStopScenario::Process(
  ...
  if (frame->reference_line_info().empty()) {
    stage_result.SetStageResult(StageStatusType::ERROR, "Reference line is empty!");
