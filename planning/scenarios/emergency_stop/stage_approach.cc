...
StageResult EmergencyStopStageApproach::Process(const TrajectoryPoint& planning_init_point, Frame* frame) {
  ...
  //wk
  const double stop_distance = (frame->GetUserintercation()->Get_value("scenario", "emergency_stop", "Stopping_distance") > 0.0)?
                                frame->GetUserintercation()->Get_value("scenario", "emergency_stop", "Stopping_distance") : 
  ...
  if (!stop_fence_exist) {
    const double deceleration = -FLAGS_longitudinal_acceleration_lower_bound; //scenario_config_.max_stop_deceleration();
  ...
  //wk
  const double stop_speed = (frame->GetUserintercation()->Get_value("scenario", "emergency_stop", "Expected_speed") > 0.0)?
                                frame->GetUserintercation()->Get_value("scenario", "emergency_stop", "Expected_speed") : 
                                max_adc_stop_speed;
  if (adc_speed <= stop_speed) {
    return FinishStage();
  }
