...
StageResult EmergencyStopStageStandby::Process(const TrajectoryPoint& planning_init_point, Frame* frame) {
  ...
  if ((pad_msg_driving_action != PadMessage::STOP &&
      frame->GetUserintercation()->Get_value("real", "trj", "", "emergency_stop") == 0.0) || frame->GetUserintercation()->Get_value("trj", "", "vehicle_launch") == 1.0) {
    //wk
    {
      std::lock_guard<std::mutex> lock(user_intercation_mutex_);
      frame->GetUserintercation()->Set_value("real", "trj", "", "emergency_stop", 0.0);
