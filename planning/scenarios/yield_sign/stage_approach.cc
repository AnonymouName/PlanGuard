...
StageResult YieldSignStageApproach::Process(
  ...
  for (const auto& yield_sign_overlap_id :
    ...
    if (distance_adc_pass_stop_sign > kPassStopLineBuffer && FLAGS_wait != 1.0) {
      // passed stop line
      return FinishStage();
    }
    ...
    //wk
    const double max_valid_stop_distance = (frame->GetUserintercation()->Get_value("scenario", "yield_signal", "Stopping_distance") > 0.0)?
                                            frame->GetUserintercation()->Get_value("scenario", "yield_signal", "Stopping_distance") :
    ...
    if (yield_sign_done && FLAGS_wait != 1.0) {
      return FinishStage();
    }
  }
