...
StageResult YieldSignStageCreep::Process(
  ...
  //wk
  const double timeout_sec = (frame->GetUserintercation()->Get_value("scenario", "yield_signal", "Creep_time") > 0.0)?
                              frame->GetUserintercation()->Get_value("scenario", "yield_signal", "Creep_time") :
                              scenario_config_.creep_timeout_sec();
