...
bool YieldSignScenario::IsTransferable(const Scenario* other_scenario,
  ...
  //wk
  const double pre_distance = (frame.GetUserintercation()->Get_value("scenario", "yield_signal", "Preparation_distance") > 0.0)?
                              frame.GetUserintercation()->Get_value("scenario", "yield_signal", "Preparation_distance") :
                              context_.scenario_config.start_yield_sign_scenario_distance();

  const bool yield_sign_scenario =
      (adc_distance_to_yield_sign > 0.0 &&
       adc_distance_to_yield_sign <= pre_distance);
           //context_.scenario_config.start_yield_sign_scenario_distance());
