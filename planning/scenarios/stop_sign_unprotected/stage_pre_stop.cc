...
StageResult StopSignUnprotectedStagePreStop::Process(
  ...
  //wk
  const double pre_stop_distance = frame->GetUserintercation()->Get_value("scenario", "stop_signal", "Stopping_distance");

  if (distance_adc_pass_stop_sign <= kPassStopLineBuffer) {
    // not passed stop line, check valid stop
    if (CheckADCStop(adc_front_edge_s, current_stop_sign_overlap->start_s, pre_stop_distance)) {
      return FinishStage();
    }
  } else {
    // passed stop line
  ...
  if (frame->GetUserintercation()->Get_value("trj", "", "vehicle_launch") == 1.0){
    return FinishStage();
  }
  ...
  //wk
  const double check_distance = frame->GetUserintercation()->Get_value("scenario", "stop_signal", "Check_distance");

  for (const auto* obstacle : path_decision.obstacles().Items()) {
    // add to watch_vehicles if adc is still proceeding to stop sign
    AddWatchVehicle(*obstacle, &watch_vehicles, check_distance);
  }
...
int StopSignUnprotectedStagePreStop::AddWatchVehicle(
    const Obstacle& obstacle, StopSignLaneVehicles* watch_vehicles, const double& check_distance) {
  ...
  //wk
  const double watch_vehicle_max_valid_stop_distance = (check_distance > 0.0)? check_distance : 
                                                        GetContextAs<StopSignUnprotectedContext>()->scenario_config.watch_vehicle_max_valid_stop_distance();

  if (distance_to_stop_line > watch_vehicle_max_valid_stop_distance) {
...
bool StopSignUnprotectedStagePreStop::CheckADCStop(
    const double adc_front_edge_s, const double stop_line_s, const double& pre_stop_distance) {
  ...
  //wk
  const double stop_distance = (pre_stop_distance > 0.0)? pre_stop_distance :
                      GetContextAs<StopSignUnprotectedContext>()->scenario_config.max_valid_stop_distance();

  if (distance_stop_line_to_adc_front_edge > stop_distance) {
      //GetContextAs<StopSignUnprotectedContext>()
