...
StageResult StopSignUnprotectedStageStop::Process(
  ...
  //wk
  const double wait_time_low = (frame->GetUserintercation()->Get_value("scenario", "stop_signal", "Wait_time") > 0.0)?
                                frame->GetUserintercation()->Get_value("scenario", "stop_signal", "Wait_time") - 1.0 :
                                scenario_config.stop_duration_sec();

  const double wait_time_up = (frame->GetUserintercation()->Get_value("scenario", "stop_signal", "Wait_time") > 0.0)?
                               frame->GetUserintercation()->Get_value("scenario", "stop_signal", "Wait_time") + 1.0 :
                                scenario_config.stop_timeout_sec();
  ...
  ADEBUG << "stop_start_time[" << start_time << "] wait_time[" << wait_time
         << "]";
  if ((wait_time < wait_time_low || FLAGS_wait == 1.0) && frame->GetUserintercation()->Get_value("trj", "", "vehicle_launch") != 1.0) {//scenario_config.stop_duration_sec()) {
  ...
  //wk
  if ((wait_time > wait_time_up || frame->GetUserintercation()->Get_value("trj", "", "vehicle_launch") == 1.0) && 
  ...
  RemoveWatchVehicle(path_decision, &watch_vehicles, check_distance);
...
int StopSignUnprotectedStageStop::RemoveWatchVehicle(
    const PathDecision& path_decision, StopSignLaneVehicles* watch_vehicles, const double& check_distance) {
  ...
  for (auto& vehicle : *watch_vehicles) {
    ...
    for (const auto& perception_obstacle_id : vehicles) {
      ...
      const double watch_vehicle_max_valid_stop_distance = (check_distance > 0.0)? check_distance : 
                                                        context->scenario_config.watch_vehicle_max_valid_stop_distance();

      if (distance > watch_vehicle_max_valid_stop_distance) {//10.0) {
        ADEBUG << "mark ERASE obstacle_id[" << perception_obstacle_id << "]";
        remove_vehicles.push_back(perception_obstacle_id);
      }
    }
