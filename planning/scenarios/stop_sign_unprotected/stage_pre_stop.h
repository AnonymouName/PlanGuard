 ...
 private:
  int AddWatchVehicle(const Obstacle& obstacle,
                      std::unordered_map<std::string, std::vector<std::string>>*
                          watch_vehicles, const double& check_distance = 0.0);

  bool CheckADCStop(const double adc_front_edge_s, const double stop_line_s, const double& pre_stop_distance = 0.0);
