...
bool StopSignUnprotectedScenario::IsTransferable(
  ...
  //wk
  const double pre_distance = (frame.GetUserintercation()->Get_value("scenario", "stop_signal", "Preparation_distance") > 0.0)?
                              frame.GetUserintercation()->Get_value("scenario", "stop_signal", "Preparation_distance") :
                              context_.scenario_config.start_stop_sign_scenario_distance();

  const bool stop_sign_scenario =
      (adc_distance_to_stop_sign > 0.0 &&
       adc_distance_to_stop_sign <= pre_distance);
           //context_.scenario_config.start_stop_sign_scenario_distance());
