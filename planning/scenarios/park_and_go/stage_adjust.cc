...
StageResult ParkAndGoStageAdjust::FinishStage() {
  ...
  //wk
  const double max_steering_percentage = (frame_->GetUserintercation()->Get_value("scenario", "open_space_launch", "Permissible_max_steering_percentage") > 0.0)?
                                          frame_->GetUserintercation()->Get_value("scenario", "open_space_launch", "Permissible_max_steering_percentage") :
                                          GetContextAs<ParkAndGoContext>()->scenario_config.max_steering_percentage_when_cruise();

  if (std::fabs(vehicle_status->steering_percentage()) < max_steering_percentage){
