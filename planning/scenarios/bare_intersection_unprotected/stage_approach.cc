...
StageResult BareIntersectionUnprotectedStageApproach::Process(
  ...
  ADEBUG << "pnc_junction_overlap_id[" << pnc_junction_overlap_id
         ...
         << "]";
  if (distance_adc_to_pnc_junction < -kPassStopLineBuffer && FLAGS_wait != 1.0) {
    // passed stop line
    {
      std::lock_guard<std::mutex> lock(user_intercation_mutex_);
      frame->GetUserintercation()->Set_value("real", "trj", "", "vehicle_launch", 0.0);
  ...
  if ((scenario_config_.enable_explicit_stop() && 
      frame->GetUserintercation()->Get_value("scenario", "intersection", "Whether_stop") == 0.0) ||
      frame->GetUserintercation()->Get_value("scenario", "intersection", "Whether_stop") == 1.0) {
    static constexpr double kCheckClearDistance = 5.0;  // meter
    static constexpr double kStartWatchDistance = 2.0;  // meter
    //wk
    const double check_distance = (frame->GetUserintercation()->Get_value("scenario", "intersection", "Check_distance") > 0.0)?
                                  frame->GetUserintercation()->Get_value("scenario", "intersection", "Check_distance") :
                                  kCheckClearDistance;
    if (distance_adc_to_pnc_junction <= check_distance &&   //kCheckClearDistance &&
      ...
      if (counter_ >= 5 || frame->GetUserintercation()->Get_value("trj", "", "vehicle_launch") == 1.0) {
        counter_ = 0;  // reset
      } else {
        stop = true;
      }
