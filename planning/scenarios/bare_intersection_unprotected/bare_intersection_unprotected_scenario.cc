...
bool BareIntersectionUnprotectedScenario::IsTransferable(
  ...
  //wk
  const double pre_distance = (frame.GetUserintercation()->Get_value("scenario", "intersection", "Preparation_distance") > 0.0)?
                              frame.GetUserintercation()->Get_value("scenario", "intersection", "Preparation_distance") :
  ...
  const bool bare_junction_scenario =
      (adc_distance_to_pnc_junction > 0.0 &&
       adc_distance_to_pnc_junction <= pre_distance);
           //context_.scenario_config
