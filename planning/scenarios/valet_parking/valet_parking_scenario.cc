...
bool ValetParkingScenario::IsTransferable(const Scenario* const other_scenario,
  ...
  if (!frame.local_view().planning_command->has_parking_command() && 
      frame.GetUserintercation()->Get_value("trj", "", "valet_park") != 1.0) {
    return false;
  }
  ...
  if (frame.local_view().planning_command->has_parking_command() &&
    ...
    target_parking_spot_id = frame.local_view()
                                 .planning_command->parking_command()
                                 .parking_spot_id();
  } else if (frame.GetUserintercation()->Get_value("trj", "", "valet_park") == 1.0 &&
      !frame.GetUserintercation()->Get_ParkId().empty()) {
    target_parking_spot_id = frame.GetUserintercation()->Get_ParkId();
  } else {
  ...
  //wk
  double parking_spot_range_to_start = (frame.GetUserintercation()->Get_value("scenario", "valet_park", "Preparation_distance") > 0.0)?
                                      frame.GetUserintercation()->Get_value("scenario", "valet_park", "Preparation_distance") :
  ...
  //wk
  {
    ...
    frame.GetUserintercation()->Set_value("real", "trj", "", "valet_park", 0.0);
    frame.GetUserintercation()->Set_value("real", "park_id", "");
  }
  return true;
}
