...
StageResult StageApproachingParkingSpot::Process(
  ...
  *(frame->mutable_open_space_info()->mutable_pre_stop_rightaway_point()) =
  ...
  //wk
  const double speed = frame->GetUserintercation()->Get_value("scenario", "valet_park", "Expected_speed");

  auto* reference_lines = frame->mutable_reference_line_info();
  for (auto& reference_line : *reference_lines) {
    //wk
    if (speed > 0.0) {
      reference_line.LimitCruiseSpeed(speed);
    }
...
//wk
  const double speed = (frame.GetUserintercation()->Get_value("scenario", "valet_park", "Expected_speed") > 0.0)?
                        frame.GetUserintercation()->Get_value("scenario", "valet_park", "Expected_speed") :
                        max_adc_stop_speed;

  if (adc_speed >  speed) {    //max_adc_stop_speed) {
  ...
  //wk
  const double distance = (frame.GetUserintercation()->Get_value("scenario", "valet_park", "Stopping_distance") > 0.0)?
                        frame.GetUserintercation()->Get_value("scenario", "valet_park", "Stopping_distance") :
                        scenario_config_.max_valid_stop_distance();

  if (distance_stop_line_to_adc_front_edge > distance) {
      //scenario_config_.max_valid_stop_distance()) {
