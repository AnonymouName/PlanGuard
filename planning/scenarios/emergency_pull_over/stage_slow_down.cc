...
StageResult EmergencyPullOverStageSlowDown::Process(const TrajectoryPoint& planning_init_point, Frame* frame) {
  ...
  if (frame->GetUserintercation()->Get_value("trj", "", "emergency_pull_over") == 1.0){
    slow_speed = frame->GetUserintercation()->Get_value("scenario", "emergency_pull_over", "Expected_speed");
  } else {
    slow_speed = frame->GetUserintercation()->Get_value("scenario", "pull_over", "Expected_speed");
  }

  if (target_slow_down_speed <= 0) {
    if (slow_speed > 0.0) {
    target_slow_down_speed = scenario_context->target_slow_down_speed = slow_speed;
    } else {
