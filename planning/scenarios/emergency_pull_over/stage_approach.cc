...
StageResult EmergencyPullOverStageApproach::Process(const TrajectoryPoint& planning_init_point, Frame* frame) {
  ...
  if (pull_over_status.has_position() && pull_over_status.position().has_x() &&
    ...
    stop_line_s = pull_over_sl.s() + stop_distance + VehicleConfigHelper::GetConfig().vehicle_param().front_edge_to_center();
