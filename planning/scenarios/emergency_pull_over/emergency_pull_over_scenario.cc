...
bool EmergencyPullOverScenario::Init(std::shared_ptr<DependencyInjector> injector, const std::string& name) {
  if (init_) {
    return true;
  }
...
bool EmergencyPullOverScenario::IsTransferable(const Scenario* const other_scenario, const Frame& frame) {
  ...
  if (pad_msg_driving_action == PadMessage::PULL_OVER ||
      frame.GetUserintercation()->Get_value("trj", "", "emergency_pull_over") == 1.0 ||
      frame.GetUserintercation()->Get_value("trj", "", "pull_over") == 1.0) {
        {
