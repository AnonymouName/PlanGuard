...
StageResult TrafficLightUnprotectedRightTurnStageCreep::Process(
  ...
  //wk
  const double timeout_sec = (frame->GetUserintercation()->Get_value("scenario", "intersection", "Creep_time") > 0.0)?
                            frame->GetUserintercation()->Get_value("scenario", "intersection", "Creep_time") : 
                            scenario_config.creep_timeout_sec();
