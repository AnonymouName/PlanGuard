...
StageResult TrafficLightUnprotectedRightTurnStageStop::Process(
  ...
  for (const auto& traffic_light_overlap_id :
    ...
    //wk
    const double approach_cruise_speed = frame->GetUserintercation()->Get_value("scenario", "intersection", "Expected_speed");
    if (approach_cruise_speed > 0.0){
    ...
    if (distance_adc_to_stop_line > stop_distance) {      //scenario_config.max_valid_stop_distance()) {
    ...
    //wk
    if (signal_color != TrafficLight::GREEN && frame->GetUserintercation()->Get_value("trj", "", "whether_stop_before_intersection") != 2.0) {
  ...
  if (traffic_light_all_stop && (traffic_light_all_green || 
      frame->GetUserintercation()->Get_value("trj", "", "vehicle_launch") == 1.0) && FLAGS_wait != 1.0) {
    {
      std::lock_guard<std::mutex> lock(user_intercation_mutex_);
      frame->GetUserintercation()->Set_value("real", "trj", "", "vehicle_launch", 0.0);
      frame->GetUserintercation()->Set_value("real", "trj", "", "whether_stop_before_intersection", 0.0);
    }
    return FinishStage(true, frame);
  }
  ...
  if (!traffic_light_no_right_turn_on_red) {
    if (traffic_light_all_stop && !traffic_light_all_green) {
      ...
      ADEBUG << "distance_adc_pass_stop_line[" << distance_adc_pass_stop_line
             << "]";
      if (distance_adc_pass_stop_line > scenario_config.min_pass_s_distance() && FLAGS_wait != 1.0) {
        //wk
        {
          std::lock_guard<std::mutex> lock(user_intercation_mutex_);
          frame->GetUserintercation()->Set_value("real", "trj", "", "vehicle_launch", 0.0);
        ...
        return FinishStage(false, frame);
      }

      //wk
      if ((scenario_config.enable_right_turn_on_red() && 
          frame->GetUserintercation()->Get_value("scenario", "traffic_light_signal", "Whether_red_light_turn_right") == 0.0) ||
          frame->GetUserintercation()->Get_value("scenario", "traffic_light_signal", "Whether_red_light_turn_right") == 1.0) {
        // when right_turn_on_red is enabled
        // check on wait-time
        if (context->stop_start_time == 0.0) {
          context->stop_start_time = Clock::NowInSeconds();
        } else {
          ...
          //wk
          if (wait_time >
              scenario_config.red_light_right_turn_stop_duration_sec() && FLAGS_wait != 1.0) {
            {
              std::lock_guard<std::mutex> lock(user_intercation_mutex_);
              frame->GetUserintercation()->Set_value("real", "trj", "", "vehicle_launch", 0.0);
              frame->GetUserintercation()->Set_value("real", "trj", "", "whether_stop_before_intersection", 0.0);
...
StageResult TrafficLightUnprotectedRightTurnStageStop::FinishStage(
    const bool protected_mode, Frame* frame) {
  auto context = GetContextAs<TrafficLightUnprotectedRightTurnContext>();
  if (protected_mode) {
    // intersection_cruise
